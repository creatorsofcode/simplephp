...
<script>
  ...
  window.csrfToken = <?php echo json_encode(simplephp_csrf_token()); ?>;

  // Attaches the CSRF token to every POST made through this helper -
  // as a FormData field, or an X-CSRF-Token header for JSON bodies.
...
?>
  ...
  <div class="top-bar">
    ...
    <div class="top-bar-right">
      <?php if(isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in'] === true): ?>
      <div class="user-greeting">Hello <?= htmlspecialchars($_SESSION['admin_username'] ?? 'admin') ?></div>
      <a href="admin/index.php?logout=1" class="logout-link">Log out</a>
      <?php endif; ?>
    </div>
  </div>
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-section">
        ...
        <div class="sidebar-content" style="padding: 12px; font-size: 13px; color: var(--color-text-light);">
          <?php if(isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in'] === true): ?>
          <p style="margin-bottom: 8px;">Logged in as <strong><?= htmlspecialchars($_SESSION['admin_username'] ?? 'admin') ?></strong></p>
          <a href="admin/index.php" style="color: var(--color-primary); text-decoration: none; font-weight: 500;">Go to Admin Panel</a>
          <?php else: ?>
          <p style="margin-bottom: 8px;">Try the demo admin panel.</p>
          <a href="admin/login.php" style="color: var(--color-primary); text-decoration: none; font-weight: 500;">Admin Login</a>
          <?php endif; ?>
        </div>
      </div>
    </aside>
  ...
  <footer>
    <div class="foot">
      ...
      <p style="font-size:12px;color:#777;" data-editable="site.demo_note">
        Note: This is a demo. Any changed data will be automatically reset back to the default state 30 seconds after your last edit.
      </p>
    </div>
  </footer>
