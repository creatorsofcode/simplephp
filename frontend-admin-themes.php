...
function loadThemeConfig(string $themeConfigFile): array
{
    if (!file_exists($themeConfigFile)) {
        ...
        simplephp_json_write($themeConfigFile, $defaultConfig);
        return $defaultConfig;
    }

    $config = simplephp_json_read($themeConfigFile, null);
    if (!is_array($config)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'set_theme') {
        ...
        try {
            ...
            if (!simplephp_json_write($themeConfigFile, $config)) {
    ...
    if ($action === 'create_theme') {
        ...
        try {
            ...
            if (!simplephp_json_write($themePath . '/theme.json', $themeData)) {
