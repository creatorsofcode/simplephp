...
function analytics_tracker_increment_visits() {
    ...
    $isFirstVisitThisSession = !isset($_SESSION['analytics_tracker_visited']);
    $_SESSION['analytics_tracker_visited'] = true;

    simplephp_json_update($counterFile, function ($counter) use ($isFirstVisitThisSession) {
        $counter['total_visits'] = ($counter['total_visits'] ?? 0) + 1;
        if ($isFirstVisitThisSession) {
            $counter['unique_visits'] = ($counter['unique_visits'] ?? 0) + 1;
        }
        return $counter;
    }, ['total_visits' => 0, 'unique_visits' => 0, 'last_reset' => date('Y-m-d H:i:s')]);
}

// Get analytics data
function analytics_tracker_get_stats() {
    $counterFile = __DIR__ . '/visit-counter.json';
    return simplephp_json_read($counterFile, [
        'total_visits' => 0,
        'unique_visits' => 0,
        'last_reset' => date('Y-m-d H:i:s')
    ]);
