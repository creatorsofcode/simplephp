...
class InlineEditor {
    ...
    async loadContent() {
        try {
            const response = await csrfFetch('get-content.php');
    ...
    async saveEdit(element) {
        ...
        try {
            const response = await csrfFetch('save-inline-edit.php', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                ...
                this.updateChangeCounter();
                this.scheduleAutoRevertNotice(result.reset_in_seconds);
            } else {
    ...
    // Demo protection: content is auto-reverted a fixed delay after the
    // last save. This lets the admin know and reloads once it happens so
    // the page reflects the reverted content without a manual refresh.
    scheduleAutoRevertNotice(seconds) {
        if (!seconds) return;
        ...
        this.showNotification(`Demo mode: this change will auto-revert in ${seconds}s`, 'info');

        this._revertTimeout = setTimeout(async () => {
            try {
                const response = await csrfFetch('check-reset.php');
                const result = await response.json();
                if (result.reset) {
                    this.showNotification('Demo content has been reset.', 'info');
